...
namespace Game2048
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            FileStream fs = new FileStream("score.txt", FileMode.OpenOrCreate, FileAccess.Read);
            StreamReader reader = new StreamReader(fs, Encoding.UTF8);
            int t;
            if (int.TryParse(reader.ReadLine(), out t))
            {
                maxScore = t;
            }
            this.textBox2.Text = maxScore.ToString();
            reader.Close();
            fs.Close();
            StartGame(false);
        }
        ...
        private void GenerateAllButtons()
        {
            ...
            for (int i = 0; i < btns.Length; i++)
            {
                ...
                Color color = GetColorOfButton(board[r, c]);
                btn.BackColor = color;
                Color color1 = Color.FromArgb(255 - color.R, 255 - color.G, 255 - color.B);
        ...
        private void RefreshAllButtons()
        {
            for (int i = 0; i < btns.Length; i++)
            {
                ...
                Color color = GetColorOfButton(board[r, c]);
                btns[i].BackColor = color;
                Color color1 = Color.FromArgb(255 - color.R, 255 - color.G, 255 - color.B);
                btns[i].ForeColor = color1;
            }
        ...
        Color GetColorOfButton(int n)
        {
            ...
            int tmp = 250-(int)Math.Log(n, 2) * 20;
            return Color.FromArgb(tmp, tmp, 0);
        }
        private bool Process(Keys keyData)
        {
            bool bMoved = false;
            ...
            if (bMoved)
            {
                ...
                if (IsGameOver() == true)
                {
                    ...
                    FileStream fs = new FileStream("score.txt", FileMode.Truncate, FileAccess.Write);
        ...
        {
            this.gameMode = this.comboBox1.SelectedIndex;
            RefreshUI();
        }
